...
public interface NhanvienRepository extends JpaRepository<Nhanvien, Integer> {
        ...
        // User entity stores role id as 'maVaiTro' (Integer), so query by that property
        ...
        AND nv.maNhanVien NOT IN (
                SELECT c.huongDanVien.maNhanVien FROM ChuyenDuLich c
                WHERE c.huongDanVien IS NOT NULL
                AND c.ngayKetThuc > :startDate AND c.ngayBatDau < :endDate
        )
        AND nv.maNhanVien NOT IN (
                SELECT c.taiXe.maNhanVien FROM ChuyenDuLich c
